...
contract esTROVE is OwnableUpgradeable, ERC20Upgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function batchMintEsTrove(address[] memory _users, uint[] memory amounts) external onlyOwner {
  ...
  function _beforeTokenTransfer(address from, address to, uint256 amount)
      ...
      override
  {
    require(((to == address(0) && from == _msgSender()) || (to == _msgSender() && from == address(0))) || (msg.sender == stakingAddress) || (msg.sender == owner()), "This token is untransferrable");
    if (to == stakingAddress) require(amount <= balanceOf(from) - vesting[from].totalAmount, "Cannot transfer vesting part");
  ...
  function userVested(address _user) public view returns (uint256 pendingReward) {
    VestInfo memory vestInfo = vesting[_user];
    uint256 accrualAmount = 0;

    for (uint i = vestInfo.amounts.length; i >= 1; i--) {
        accrualAmount += min(vestInfo.amounts[i-1], vestInfo.amounts[i-1] * (block.timestamp - vestInfo.stakedAt[i-1]) / 31_104_000);
    }
  ...
  function vest(uint256 _amount) external whenNotPaused nonReentrant {
    ...
    require(vesting[_msgSender()].amounts.length < maxStakes, "maxStakes exceeded");
    require(balanceOf(_msgSender()) - vesting[_msgSender()].totalAmount >= _amount, "not enough free esTROVE to vest");
  ...
  function unvest(uint256 amount) external whenNotPaused nonReentrant {
    require(vesting[_msgSender()].totalAmount >= amount, "insufficient staked amount");
    ...
    if (vesting[_msgSender()].totalAmount <= 0) usersEnabled[_msgSender()] = false;

    require(ERC20Upgradeable(trove).transfer(_msgSender(), actualVested) == true, "withdraw failed."); //3.75
